...
function PopupNumbers(target, pfx, color, lifetime, number, presymbol, postsymbol,boolean)
    ...
    if boolean == false then
    	...
    	pidx = ParticleManager:CreateParticle(pfxPath, PATTACH_OVERHEAD_FOLLOW, target) -- target:GetOwner()
    end
...
function AMHC:GivePlayerGold_Reliable( playerid,gold )
	local player = PlayerResource:GetPlayer(playerid)
...
function AMHC:GivePlayerGold_UnReliable( playerid,gold )
	local player = PlayerResource:GetPlayer(playerid)
	...
	if player then
		...
		if hero then
			--hero:EmitSoundParams("General.Sell",200,200,0.1)
			PlayerResource:SetGold(playerid,PlayerResource:GetGold(playerid) + gold,false)
			_G.PlayerEarnedGold[playerid] = _G.PlayerEarnedGold[playerid] + gold
			self:CreateNumberEffect( hero,gold,1,self.MSG_GOLD,"yellow",0 )
		end
	end
