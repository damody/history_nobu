...
--[[Author: YOLOSPAGHETTI
    ...
    function moon_mirror_Jump(keys)
        ...
        target:RemoveModifierByName("modifier_arc_moon_mirror")
        local count = 0
        ...
        if ability.jump_count[current] > 0 then
            ...
            if new_target ~= nil then
                ...
                if not new_target:FindModifierByName("modifier_arc_moon_mirror_cd") then
                    ability:ApplyDataDrivenModifier(caster, new_target, "modifier_arc_moon_mirror", {})
                    ability:ApplyDataDrivenModifier(caster, new_target, "modifier_arc_moon_mirror_cd", {duration=heal_cd})
    ...
    function moon_mirror(keys)
        local caster = keys.caster
        local ability = keys.ability
        local target = keys.target
        local heal_cd = ability:GetSpecialValueFor("heal_cd")
        if not target:FindModifierByName("modifier_arc_moon_mirror_cd") then
            ability:ApplyDataDrivenModifier(caster, target, "modifier_arc_moon_mirror", {})
            ability:ApplyDataDrivenModifier(caster, target, "modifier_arc_moon_mirror_cd", {duration=heal_cd})
            ability:ApplyDataDrivenModifier(caster, target, "modifier_arc_reito_lantern_cd", {duration=heal_cd})
        end
