...
function VectorTarget:StartVectorCast( event )
	...
	if ability then
		...
		local function OverrideSpellStart(ability, position, direction)
			ability:OnVectorCastStart(position, direction)
		end
		unit.inVectorCast = nil
		unit.isVectorCasting = false
		ability.OnSpellStart = function(ability) return OverrideSpellStart(ability, position, direction) end
	end
end
