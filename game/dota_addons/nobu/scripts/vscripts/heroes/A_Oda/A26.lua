...
function A26R_OnAttackStart( keys )
	...
	if crit_chance >= rnd or caster.A26R_count > (100/crit_chance) then
		caster.A26R_count = 0
		if caster.maximum_critical_damage < caster:GetAverageTrueAttackDamage(target) * 2 then
			caster.maximum_critical_damage = caster:GetAverageTrueAttackDamage(target) * 2
		end
...
function A26T_OnAttackLanded( keys )
	local caster = keys.caster
	local target = keys.target
	local point = target:GetAbsOrigin()
	local ability = keys.ability
	local damage = ability:GetSpecialValueFor("small_bomb_damage")
...
function A26T_small_bomb_OnHit( keys )
	local caster = keys.caster
	local target = keys.target
...
function A26T_OnProjectileHitUnit( keys )
	...
	-- change to small bomb
	if ability.small_bomb then
		A26T_small_bomb_OnHit( keys )
		return 
	end
	-- remove spell hint
	ability.dummy:RemoveModifierByName("nobu_modifier_spell_hint")
	-- 搜尋
	local units = FindUnitsInRadius(caster:GetTeamNumber(),	-- 關係參考
		center,							-- 搜尋的中心點
		nil,
