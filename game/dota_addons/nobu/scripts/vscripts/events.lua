...
function Nobu:FilterGold( filterTable )
  ...
  if reason == DOTA_ModifyGold_Building then
    if gold ~= 150 and gold ~= 750 and gold ~= 50 and gold ~= 800 and gold ~= 1300 and gold ~= 300 then return false end
    if gold == 1300 then
      local hero = _G.Hero[playerID]
      give_money_for_together_hero(hero, 300, 2000)
    elseif gold == 750 then 
      filterTable["gold"] = 750
      _G.PlayerEarnedGold[playerID] = _G.PlayerEarnedGold[playerID] + 750
    else
...
function Nobu:HealFilter(keys)
  ...
  if healer then
    if healer ~= target then
      ...
      if healer.HasModifier and healer:HasModifier("modifier_item_reito_lantern") ~= nil then
  ...
  if target:HasModifier("modifier_C07T_2") then
    if target.decrease_health then
      if target.decrease_health >= 0.5 then
        target.decrease_health = 0.5
      end
